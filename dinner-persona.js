...
(function (root) {
  ...
  // 中学生にもわかる言葉で、1軸1行。
  const features = {
    joy: { L: ["😋", "見た目より、味が決め手"], R: ["🌈", "見た目の彩りでテンションが上がる"], "=": ["⚖️", "味も見た目も、どっちも大事"] },
    budget: { L: ["💰", "節約じょうず。あるものでやりくり"], R: ["🥦", "いろんな食材を食べたい"], "=": ["⚖️", "節約も栄養も、ほどほどに"] },
    effort: { L: ["⚡", "早く食べたい！ 時短が正義"], R: ["🍳", "料理の時間も楽しみたい"], "=": ["⚖️", "早さも手間も、日によって"] },
  };
  ...
  function result(raw) {
    ...
    return {
      ...
      conversation: info[4],
      dimensions,
      features: dimensions.filter((d) => features[d.id][d.pole]).map((d) => features[d.id][d.pole]),
